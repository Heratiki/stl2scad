...
def test_stl_metrics_calculation(sample_stl_file):
    ...
    assert (
        3 < volume_to_area_ratio < 10
    ), "Volume to area ratio should be in a reasonable range"
...
def test_conversion_and_verification(sample_stl_file, test_output_dir):
    ...
    assert (
        "surface_area" in result.comparison
    ), "Surface area comparison should be present"
    assert (
        "bounding_box" in result.comparison
    ), "Bounding box comparison should be present"
...
def test_visualization_generation(sample_stl_file, test_output_dir):
    ...
    try:
        ...
        assert visualizations[
            "perspective"
        ].exists(), "Perspective view file should exist"
    ...
    for stl_file in stl_files:
        ...
        try:
            ...
            result = verify_existing_conversion(
                stl_file,
                scad_file,
                tolerance={"volume": 5.0, "surface_area": 10.0, "bounding_box": 2.0},
            )
            ...
            assert (
                result is not None
            ), f"Verification result for {stl_file.name} should not be None"
...
def test_phase2_metrics(test_data_dir, test_output_dir):
    ...
    from stl2scad.core.verification.metrics import (
        calculate_hausdorff_distance,
        compare_normal_vectors,
        sample_mesh_points,
    )
...
def test_compare_metrics_deterministic_with_sample_seed(test_output_dir):
    ...
    assert (
        abs(
            result_1["hausdorff_distance"]["value"]
            - result_3["hausdorff_distance"]["value"]
        )
        > 1e-9
    )
