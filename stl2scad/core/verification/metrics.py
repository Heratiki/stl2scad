...
def compare_normal_vectors(
    points1: np.ndarray, normals1: np.ndarray, points2: np.ndarray, normals2: np.ndarray
) -> float:
    """
...
def calculate_scad_metrics(
    scad_file: Union[str, Path], timeout: int = 120
) -> Dict[str, Any]:
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to calculate SCAD metrics after rendering: {str(e)}"
            )
...
def compare_metrics(
    ...
    # Compare volume
    if (
        "volume" in stl_metrics
        and "volume" in scad_metrics
        and stl_metrics["volume"] is not None
        and scad_metrics["volume"] is not None
    ):
        stl_volume = stl_metrics["volume"]
        scad_volume = scad_metrics["volume"]
        volume_diff = scad_volume - stl_volume
        volume_diff_percent = (
            (volume_diff / stl_volume) * 100 if stl_volume != 0 else float("inf")
        )
    ...
    if (
        "surface_area" in stl_metrics
        and "surface_area" in scad_metrics
        and stl_metrics["surface_area"] is not None
        and scad_metrics["surface_area"] is not None
    ):
        stl_area = stl_metrics["surface_area"]
        scad_area = scad_metrics["surface_area"]
        area_diff = scad_area - stl_area
        area_diff_percent = (
            (area_diff / stl_area) * 100 if stl_area != 0 else float("inf")
        )
    ...
    if (
        "bounding_box" in stl_metrics
        and "bounding_box" in scad_metrics
        and stl_metrics["bounding_box"] is not None
        and scad_metrics["bounding_box"] is not None
    ):
        ...
        for dimension in ["width", "height", "depth"]:
            if dimension in stl_bbox and dimension in scad_bbox:
                ...
                dim_diff_percent = (
                    (dim_diff / stl_dim) * 100 if stl_dim != 0 else float("inf")
                )
    ...
    if (
        "mesh" in stl_metrics
        and "mesh" in scad_metrics
        and stl_metrics["mesh"] is not None
        and scad_metrics["mesh"] is not None
    ):
        ...
        rng_scad = np.random.default_rng(
            None if sample_seed is None else sample_seed + 1
        )

        num_samples = 1000
        stl_points, stl_normals = sample_mesh_points(stl_mesh, num_samples, rng=rng_stl)
        scad_points, scad_normals = sample_mesh_points(
            scad_mesh, num_samples, rng=rng_scad
        )

        hausdorff = calculate_hausdorff_distance(stl_points, scad_points)
        normal_dev = compare_normal_vectors(
            stl_points, stl_normals, scad_points, scad_normals
        )
        ...
        stl_bbox = stl_metrics["bounding_box"]
        diagonal = np.sqrt(
            stl_bbox["width"] ** 2 + stl_bbox["height"] ** 2 + stl_bbox["depth"] ** 2
        )
